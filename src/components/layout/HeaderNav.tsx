...
export function HeaderNav({ nav, contact, overlay = false }: HeaderNavProps) {
  ...
  return (
    <header className="sticky top-0 z-50 md:top-[-36px]">
      ...
      >
        <Container className="flex h-16 items-center justify-between gap-2 md:h-20 xl:gap-6">
          <span className="hidden lg:inline-flex">
            <Logo variant={inverted ? "white" : "navy"} width={234} priority />
          </span>
          <span className="inline-flex lg:hidden">
            <Logo variant={inverted ? "white" : "navy"} width={159} priority />
          </span>
